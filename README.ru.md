...
- **Batch-перевод** — до 50 текстов за один запрос; элементы выполняются параллельно и падают независимо друг от друга
...
Перевод до 50 текстов за один запрос. Элементы обрабатываются параллельно — ошибка одного не отменяет остальные. Каждый элемент поддерживает те же поля, что и одиночный запрос, включая необязательный `"provider"`.
